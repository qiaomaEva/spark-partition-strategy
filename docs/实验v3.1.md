...
```bash
# uniform, 2m, p16（完全均匀，skew-ratio=0.0）
python3 code/data_gen/DataGen.py \
    --type uniform \
    --num-records 2000000 \
    --num-partitions 16 \
    --skew-ratio 0.0
...
```bash
# skewed, 5m, p64，温和倾斜，skew-ratio≈0.75
python3 code/data_gen/DataGen.py \
    --type skewed \
    --num-records 5000000 \
    --num-partitions 64 \
...
#### 2.1 跑 Hash / Range / Custom（b4 / b8 / b32）
...
# Custom, skewed, 5m, p64, 单热点 key=0，b4
code/scripts/run_custom_basic.sh \
    --data-type skewed \
    --num-records 5000000 \
    --skew-ratio 0.75 \
...
# Custom, skewed, 5m, p64，单热点 key=0，b8
code/scripts/run_custom_basic.sh \
    --data-type skewed \
    --num-records 5000000 \
    --skew-ratio 0.95 \
