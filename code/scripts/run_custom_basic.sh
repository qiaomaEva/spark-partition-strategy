...
while [ $i -lt ${#ARGS[@]} ]; do
  ...
  case "$arg" in
    ...
    --hot-keys)
      ...
      FORWARDED_ARGS+=("--hot-keys" "${ARGS[$((i+1))]}")
      i=$((i+1))
      ;;
    --hot-bucket-factor)
      bucket_tag_val="${ARGS[$((i+1))]}"
      bucket_tag="b${bucket_tag_val}"
      FORWARDED_ARGS+=("--hot-bucket-factor" "${bucket_tag_val}")
      i=$((i+1))
      ;;
    *)
      FORWARDED_ARGS+=("${arg}")
...
echo "[run_custom_basic] Using dataset: ${dataset_name}"
echo "[run_custom_basic] Resolved input-path: ${input_path}"

# 2. 运行 Custom 实验（自动补全 --input-path 与 --input-type）
...
# 3. 自动解析 Tag
# 结果 JSON 命名为：
#   custom_<dataset_name>_<hotXXX>_<bX>_p<num_partitions>_timestamp.json
TAG="${dataset_name}_${hot_keys_tag}_${bucket_tag}_${num_partitions}"
